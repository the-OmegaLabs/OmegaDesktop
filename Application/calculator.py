...
class Application():
    ...
    def __init__(self, args):
        ...
        self.UI_WIDTH     = self.getScaled(320)
        self.UI_HEIGHT    = self.getScaled(415) 
        ...
        self.IMG_icon_logo   = './Resources/icons/calculator.png'
    ...
    def loadWidget(self):
        self.WDG_result_bg = maliang.Label(self.cv, justify='right', size=(self.UI_WIDTH, self.getScaled(75)), position=(0, self.getScaled(3)))
        self.WDG_result    = maliang.Text(self.WDG_result_bg, position=(self.WDG_result_bg.size[0] - self.getScaled(15), self.WDG_result_bg.size[1] // 2), anchor='e', family='MIUI EX', weight='bold', fontsize=self.getScaled(30), text='0')

        maliang.Env.system = 'Windows11' # enable round button
        buttons = []
        for i in range(6):
            line = []
            for j in range(4):
                line.append(maliang.Button(self.cv, position=(self.getScaled(4) + self.getScaled(75) * j + self.getScaled(5) * j, self.getScaled(85) + self.getScaled(50) * i + self.getScaled(5) * i), size=(self.getScaled(75), self.getScaled(50))))
            buttons.append(line)

        for i in range(2):
            for j in buttons[i]:
                j.style.set(bg=('#323232'))


        buttons[-1][-1].style.set(bg=('#0793FF')) # sumbit button
